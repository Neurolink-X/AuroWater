  ...
  return withAuth(
    ...
    async (req, user) => {
      try {
        ...
        const totalTechnicians = await queryOne<{ count: number }>(
        ...
        const revenueDaily = await query(
          `SELECT to_char(created_at::date, 'YYYY-MM-DD') as day, COALESCE(SUM(total_amount),0)::float as total
           FROM orders
        ...
        const recentOrders = await query(
          `SELECT o.id, o.customer_id, o.total_amount, o.status, o.created_at,
                  u.full_name as customer_name, st.name as service_name
           FROM orders o
        ...
        return NextResponse.json(errorResponse(error.message || 'Failed to fetch dashboard'), {
          status: 500,
        });
      }
    },
